...
psyco_curs_mogrify(cursorObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    /* note that we don't overwrite the last query executed on the cursor, we
       ...
       TODO: refactor the common mogrification code (see psycoev_curs_execute
...
_psyco_curs_prefetch(cursorObject *self)
{
    ...
    if (self->pgres == NULL || self->needsfetch) {
        ...
        do {
            i = pq_fetch(self);
            Dprintf("_psycoev_curs_prefetch: result = %d", i);
        } while(i == 1);
    }
...
psyco_curs_copy_from(cursorObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    quoted_delimiter = psycoev_escape_string((PyObject*)self->conn, sep, 0, NULL, NULL);
    ...
    query = query_buffer;
    if (null) {
        char *quoted_null = psycoev_escape_string((PyObject*)self->conn, null, 0, NULL, NULL);
        if (quoted_null == NULL) {
...
cursor_setup(cursorObject *self, connectionObject *conn, const char *name)
{
    ...
    if (PyObject_IsInstance((PyObject*)conn,
        ...
        PyErr_SetString(PyExc_TypeError,
            "argument 1 must be subclass of psycoev._psycoev.connection");
        return 1;
    } */
    Py_INCREF(conn);
    self->conn = conn;
