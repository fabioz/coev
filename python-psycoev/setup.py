...
sources = [
    'psycoevmodule.c', 'pqpath.c',  'typecast.c',
...
if have_pydatetime and (use_pydatetime or not have_mxdatetime):
    ...
    e("psycoev requires a datetime module:")
    e("    mx.DateTime module not found")
    e("    python datetime module not found")
    e("Note that psycoev needs the module headers and not just the module")
...
if version_flags:
    PSYCOEV_VERSION_EX = PSYCOEV_VERSION + " (%s)" % ' '.join(version_flags)
else:
    PSYCOEV_VERSION_EX = PSYCOEV_VERSION

if not PLATFORM_IS_WINDOWS:
    define_macros.append(('PSYCOPG_VERSION', '"'+PSYCOEV_VERSION_EX+'"'))
else:
    define_macros.append(('PSYCOPG_VERSION', '\\"'+PSYCOEV_VERSION_EX+'\\"'))
...
sources = map(lambda x: os.path.join('psycoev', x), sources)

ext.append(Extension("psycoev._psycoev", sources,
...
setup(name="psycoev",
      ...
      package_dir={'psycoev':'lib'},
      packages=['psycoev'],
      cmdclass={ 'build_ext': psycoev_build_ext },
      ext_modules=ext)
