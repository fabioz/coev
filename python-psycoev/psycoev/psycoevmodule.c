/* psycoevmodule.c - psycoev module (will import other C classes)
 *
...
#include "psycoev/lobject.h"
#include "psycoev/typecast.h"
#include "psycoev/microprotocols.h"
#include "psycoev/microprotocols_proto.h"

#include "psycoev/adapter_qstring.h"
#include "psycoev/adapter_binary.h"
#include "psycoev/adapter_pboolean.h"
#include "psycoev/adapter_pfloat.h"
#include "psycoev/adapter_asis.h"
#include "psycoev/adapter_list.h"
#include "psycoev/typecast_binary.h"

#ifdef HAVE_MXDATETIME
#include <mxDateTime.h>
#include "psycoev/adapter_mxdatetime.h"
HIDDEN mxDateTimeModule_APIObject *mxDateTimeP = NULL;
#endif

/* some module-level variables, like the datetime module */
#include <datetime.h>
#include "psycoev/adapter_datetime.h"
...
/* pointers to the psycoev.tz classes */
...
#ifdef PSYCOPG_DEBUG
HIDDEN int psycoev_debug_enabled = 0;
#endif
...
"    psycoev.connect(\"dbname=xxx user=xxx ...\")\n\n"   \
"If ``dsn`` is not provided it is possible to pass the parameters as\n"    \
"keyword arguments; e.g.::\n\n"                                            \
"    psycoev.connect(database='xxx', user='xxx', ...)\n\n"                \
...
psyco_connect(PyObject *self, PyObject *args, PyObject *keywds)
{
    ...
    if (dsn_static == NULL) {
        ...
        dsn_dynamic = PyMem_Malloc(l*sizeof(char));
        if (dsn_dynamic == NULL) {
            PyErr_NoMemory();
            goto fail;
        }
...
"register_type(obj) -> None -- register obj with psycoev type system\n\n" \
":Parameters:\n" \
...
#ifdef HAVE_MXDATETIME
    /* as above, we use the callable objects from the psycoev module */
...
static struct {
    ...
    { "psycoev.Error", &Error, 0, Error_doc },
    { "psycoev.Warning", &Warning, 0, Warning_doc },
    { "psycoev.InterfaceError", &InterfaceError, &Error, InterfaceError_doc },
    { "psycoev.DatabaseError", &DatabaseError, &Error, DatabaseError_doc },
    { "psycoev.InternalError", &InternalError, &DatabaseError, InternalError_doc },
    { "psycoev.OperationalError", &OperationalError, &DatabaseError,
        OperationalError_doc },
    { "psycoev.ProgrammingError", &ProgrammingError, &DatabaseError,
        ProgrammingError_doc },
    { "psycoev.IntegrityError", &IntegrityError, &DatabaseError,
        IntegrityError_doc },
    { "psycoev.DataError", &DataError, &DatabaseError, DataError_doc },
    { "psycoev.NotSupportedError", &NotSupportedError, &DatabaseError,
        NotSupportedError_doc },
#ifdef PSYCOPG_EXTENSIONS
    { "psycoev.extensions.QueryCanceledError", &QueryCanceledError,
      &OperationalError, OperationalError_doc },
    { "psycoev.extensions.TransactionRollbackError",
...
psyco_errors_init(void)
{
    /* the names of the exceptions here reflect the oranization of the
       psycoev module and not the fact the the original error objects
       live in _psycoev */
    ...
    /* Make pgerror, pgcode and cursor default to None on psycoev
...
static PyMethodDef psycoevMethods[] = {
...
init_psycoev(void)
{
    ...
    if (getenv("PSYCOPG_DEBUG"))
        psycoev_debug_enabled = 1;
#endif

    Dprintf("initpsycoev: initializing psycoev %s", PSYCOPG_VERSION);
    ...
    if (mxDateTime_ImportModuleAndAPI() != 0) {
        Dprintf("initpsycoev: why marc hide mx.DateTime again?!");
    ...
    if (pyDateTimeModuleP == NULL) {
        Dprintf("initpsycoev: can't import datetime module");
    ...
    /* import psycoev.tz anyway (TODO: replace with C-level module?) */
    pyPsycopgTzModule = PyImport_ImportModule("psycoev.tz");
    if (pyPsycopgTzModule == NULL) {
        Dprintf("initpsycoev: can't import psycoev.tz module");
        PyErr_SetString(PyExc_ImportError, "can't import psycoev.tz module");
        return;
    }
    pyPsycopgTzLOCAL =
    ...
    module = Py_InitModule("_psycoev", psycoevMethods);
    ...
    PyModule_AddStringConstant(module, "__doc__", "psycoev PostgreSQL driver");
