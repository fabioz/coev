...
#ifdef PSYCOPG_DEBUG
extern HIDDEN int psycoev_debug_enabled;
#endif
 ...
 * available, which replaces the "normal" strdup.  If we replace all psycoev
...
static int pthread_mutex_init(pthread_mutex_t *mutex, void* fake)
{
        *mutex = create_sem(1, "psycoev_mutex");
        if (*mutex < B_OK)
                return *mutex;
        return 0;
}
